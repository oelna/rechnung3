...
function readPanelNumber(input) {
...
$$('[data-margin]').forEach((input) => {
	input.oninput = () => {
		doc.settings.guides.margins[input.dataset.margin] = readPanelNumber(input);
		changed();
	};
});
...
$$('[data-transform]').forEach((input) => {
	input.addEventListener('keydown', (event) => {
		...
		input.value = String(readPanelNumber(input) + direction * 10);
...
$$('[data-transform]').forEach((input) => {
	input.oninput = () => {
		...
		const key = input.dataset.transform;
		frame[key] = input.type === 'checkbox' ? input.checked : readPanelNumber(input);
		changed();
	};
});
...
$$('[data-type-style]').forEach((input) => {
	input.oninput = () => {
		...
		if (input.dataset.typeStyle === 'fontSize' || input.dataset.typeStyle === 'lineHeight') {
			frame.style[input.dataset.typeStyle] = readPanelNumber(input);
		} else {
...
$('#discountType').onchange = $('#discountValue').oninput = () => {
	...
	if (frame?.type === 'table') {
		frame.content.discount = {
			...
			value: readPanelNumber($('#discountValue')),
		};
		changed();
	}
};
...
$('#documentTitle').oninput = () => {
	doc.title = $('#documentTitle').value.trim() || 'Untitled invoice';
	changed();
};
