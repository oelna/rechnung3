...
function normalizePanelNumberInput(input) {
...
$('#documentCanvas').addEventListener('input', (event) => {
	...
	if (tableCell) {
		...
		const value = tableCell.innerText.replace(/\n$/, '');
...
$$('[data-margin]').forEach((input) => {
	input.oninput = () => {
		normalizePanelNumberInput(input);
		doc.settings.guides.margins[input.dataset.margin] = parsePanelNumber(input.value);
		changed();
	};
});
...
$$('[data-transform]').forEach((input) => {
	input.addEventListener('keydown', (event) => {
		...
		input.value = String(parsePanelNumber(input.value) + direction * 10);
...
$$('[data-transform]').forEach((input) => {
	input.oninput = () => {
		...
		const key = input.dataset.transform;
		normalizePanelNumberInput(input);
		frame[key] = input.type === 'checkbox' ? input.checked : parsePanelNumber(input.value);
		changed();
	};
});
...
$$('[data-type-style]').forEach((input) => {
	input.oninput = () => {
		...
		if (input.dataset.typeStyle === 'fontSize' || input.dataset.typeStyle === 'lineHeight') {
			normalizePanelNumberInput(input);
			frame.style[input.dataset.typeStyle] = parsePanelNumber(input.value);
		} else {
			frame.style[input.dataset.typeStyle] = input.value;
		}
		changed();
	};
});
...
$('#discountType').onchange = $('#discountValue').oninput = () => {
	...
	if (frame?.type === 'table') {
		normalizePanelNumberInput($('#discountValue'));
		frame.content.discount = {
			type: $('#discountType').value,
			value: parsePanelNumber($('#discountValue').value),
		};
		changed();
	}
};
...
$('#documentCanvas').addEventListener('keydown', (event) => {
	...
	const frame = cell.closest('.frame');
	const cells = [...frame.querySelectorAll('td[data-row]')];
	...
	if (next) {
		event.preventDefault();
		next.focus();
		selectCell(frame.dataset.frameId, Number(next.dataset.row), Number(next.dataset.col));
		syncPalettes();
	}
});
